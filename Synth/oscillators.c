...
void SineTable_init(void) {
	for (uint16_t i = 0; i < SINETABLE_SIZE; i++) {
		sinetable[i] = sinf(_2PI / (SINETABLE_SIZE - 1) * i);
	}
}
...
float_t Drifters_sampleCompute(void) // 3 drifting sawtooths with "fixed" main pitch in centralFreq variable
{
	...
	y = 0.33f
			* (MorphingSaw_SampleCompute(&op1) + MorphingSaw_SampleCompute(&op2)
					+ MorphingSaw_SampleCompute(&op3));
...
float_t OpSampleCompute1(Oscillator_t *op) // basic sawtooth^2
{
	...
	if (op->phase < _PI)
		op->out = 0.5f * op->amp * (op->phase / _PI) * (op->phase / _PI);
	else
		op->out = -.5f
				* (op->amp * (op->phase / _PI - 2) * (op->phase / _PI - 2));
...
float_t OpSampleCompute2(Oscillator_t *op) // basic sawtooth
{
	...
	if (op->phase < _PI)
		op->out = 0.5f * op->amp * (op->phase / _PI);
	else
		op->out = .5f * op->amp * (op->phase / _PI - 2);
...
float_t OpSampleCompute5(Oscillator_t *op) // Naive Triangle
{
	...
	if (op->phase < _PI)
		op->out = op->amp * (-2 / _PI * op->phase + 1);
	else
		op->out = op->amp * (2 / _PI * op->phase - 3);
...
float_t MorphingSaw_SampleCompute(Oscillator_t *op) // Morphing sawtooth, tends to a triangle at high freqs
{
	...
	if (op->phi0 <= .0001f)
		op->phi0 = .0001f;
	if (op->phase < op->phi0)
		op->out = op->amp * (-2 / op->phi0 * op->phase + 1);
	else
		op->out = op->amp * (2 * op->phase - _2PI - op->phi0)
				/ (_2PI - op->phi0);
