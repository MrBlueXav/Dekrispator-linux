...
void capture_midiController(snd_seq_t *seq) {
	...
	sender.client = MIDI_CONTROLLER_ID;
	sender.port = MIDI_CONTROLLER_PORT;
	dest.client = DEKRISPATOR_ID;
	dest.port = DEKRISPATOR_PORT;
...
void midi_open(void) {
	...
	CHK(snd_seq_set_client_name(seq_handle, "Dekrispator"), "Could not set client name");
	CHK(
...
void midi_process(const snd_seq_event_t *ev) {
	if ((ev->type == SND_SEQ_EVENT_NOTEON) || (ev->type == SND_SEQ_EVENT_NOTEOFF)) {
		...
		printf("[%d] Note %s: %d vel(%d)\n", ev->time.tick, type, ev->data.note.note,
				ev->data.note.velocity);
	} else if (ev->type == SND_SEQ_EVENT_CONTROLLER)
		printf("[%d] Control:  %d val(%d)\n", ev->time.tick, ev->data.control.param,
	...
	if (sequencerIsOn == false) {
		...
		if (ev->type == SND_SEQ_EVENT_NOTEOFF) // Note off ? -------------------------------
				{
			uint8_t noteOff = ev->data.note.note;
			...
			//}
		} else if (ev->type == SND_SEQ_EVENT_NOTEON) // Note on ----------------------------
				{
			uint8_t noteOn = ev->data.note.note;
		...
		} else if (ev->type == SND_SEQ_EVENT_KEYPRESS) // Aftertouch
				{
	...
	if (ev->type == SND_SEQ_EVENT_CONTROLLER) /* If incoming midi message is a Control Change... */
	{
		uint8_t val = ev->data.control.value;

		switch (ev->data.control.param) // CC number
		{
		case 1:
			VibratoAmp_set(val);
