 ...
 * @version This code is based on alsa-lib/test/pcm.c example
 * @date
 * @brief   Dekrispator_linux main file
 ******************************************************************************
 *
 *
 ******************************************************************************
 * Run :  ./dekrispator-linux    (default args are -v -r48000 -m async)
...
static unsigned int rate = SAMPLERATE; /* stream rate */
static unsigned int channels = 2; /* count of channels, don't change ! */
...
static void generate_sound(const snd_pcm_channel_area_t *areas, snd_pcm_uframes_t offset, int count) {
	...
	while (count-- > 0) {
		...
		if (to_unsigned) {
			res_L ^= 1U << (format_bits - 1);
			res_R ^= 1U << (format_bits - 1);
		}
		/* Generate data in native endian format */
		if (big_endian) {
			for (i = 0; i < bps; i++) {
				*(samples[0] + phys_bps - 1 - i) = (res_L >> i * 8) & 0xff;
				*(samples[1] + phys_bps - 1 - i) = (res_R >> i * 8) & 0xff;
			}
		} else {
			for (i = 0; i < bps; i++) {
				*(samples[0] + i) = (res_L >> i * 8) & 0xff;
				*(samples[1] + i) = (res_R >> i * 8) & 0xff;
			}
		}
		samples[0] += steps[0];
		samples[1] += steps[1];
...
static int write_loop(snd_pcm_t *handle, signed short *samples, snd_pcm_channel_area_t *areas) {
	...
	while (1) {
		generate_sound(areas, 0, period_size);
		ptr = samples;
		cptr = period_size;
		while (cptr > 0) {
...
static void async_direct_callback(snd_async_handler_t *ahandler) {
	...
	while (1) {
		...
		while (size > 0) {
			...
			generate_sound(my_areas, offset, frames);
...
static struct transfer_method transfer_methods[] = { { "write", SND_PCM_ACCESS_RW_INTERLEAVED,
		write_loop }, { "write_and_poll", SND_PCM_ACCESS_RW_INTERLEAVED, write_and_poll_loop }, {
		"async", SND_PCM_ACCESS_RW_INTERLEAVED, async_loop }, { "async_direct",
		SND_PCM_ACCESS_MMAP_INTERLEAVED, async_direct_loop }, { "direct_interleaved",
		SND_PCM_ACCESS_MMAP_INTERLEAVED, direct_loop }, { "direct_noninterleaved",
		SND_PCM_ACCESS_MMAP_NONINTERLEAVED, direct_loop }, { "direct_write",
		SND_PCM_ACCESS_MMAP_INTERLEAVED, direct_write_loop }, { NULL, SND_PCM_ACCESS_RW_INTERLEAVED,
NULL } };
...
int main(int argc, char *argv[]) {
	...
	NULL, 'D' }, { "rate", 1, NULL, 'r' }, { "channels", 1, NULL, 'c' },
			{ "frequency", 1, NULL, 'f' }, { "buffer", 1, NULL, 'b' }, { "period", 1, NULL, 'p' }, {
					"method", 1, NULL, 'm' }, { "format", 1, NULL, 'o' },
			{ "verbose", 1, NULL, 'v' }, { "noresample", 1, NULL, 'n' }, { "pevent", 1, NULL, 'e' },
			{
			NULL, 0, NULL, 0 }, };

	int err, morehelp;
	int method = 2; // default method = async
	unsigned int chn;
	...
	while (1) {
		int c;
		if ((c = getopt_long(argc, argv, "hD:r:c:f:b:p:m:o:vne", long_option,
		NULL)) < 0)
			break;
		switch (c) {
		case 'h':
		...
		case 'm':
			for (method = 0; transfer_methods[method].name; method++)
				if (!strcmp(transfer_methods[method].name, optarg))
					break;
			if (transfer_methods[method].name == NULL)
				method = 2; // default method = async
			break;
		case 'o':
			for (format = 0; format < SND_PCM_FORMAT_LAST; format++) {
				const char *format_name = snd_pcm_format_name(format);
				if (format_name)
					if (!strcmp(format_name, optarg))
						break;
			}
