...
static void generate_sound(const snd_pcm_channel_area_t *areas,
...
static int write_loop(snd_pcm_t *handle, signed short *samples,
	...
	while (1) {
		generate_sound(areas, 0, period_size, &phase);
		ptr = samples;
		cptr = period_size;
		while (cptr > 0) {
...
static void async_callback(snd_async_handler_t *ahandler) {
	...
	struct async_private_data *data = snd_async_handler_get_callback_private(
			ahandler);
	...
	while (avail >= period_size) {
		generate_sound(areas, 0, period_size, &data->phase);
...
static void async_direct_callback(snd_async_handler_t *ahandler) {
	...
	while (1) {
		...
		while (size > 0) {
			...
			generate_sound(my_areas, offset, frames, &data->phase);
...
static int async_direct_loop(snd_pcm_t *handle,
	...
	data.phase = 0;
	err = snd_async_add_pcm_handler(&ahandler, handle, async_direct_callback,
			&data);
	if (err < 0) {
		printf("Unable to register async handler\n");
		exit(EXIT_FAILURE);
...
static int direct_loop(snd_pcm_t *handle,
	...
	while (1) {
		...
		while (size > 0) {
			...
			generate_sound(my_areas, offset, frames, &phase);
			commitres = snd_pcm_mmap_commit(handle, offset, frames);
			if (commitres < 0 || (snd_pcm_uframes_t) commitres != frames) {
				if ((err = xrun_recovery(handle,
						commitres >= 0 ? -EPIPE : commitres)) < 0) {
...
int main(int argc, char *argv[]) {
	...
	NULL, 'D' }, { "rate", 1, NULL, 'r' }, { "channels", 1, NULL, 'c' }, {
			"frequency", 1, NULL, 'f' }, { "buffer", 1, NULL, 'b' }, { "period",
			1, NULL, 'p' }, { "method", 1, NULL, 'm' },
			{ "format", 1, NULL, 'o' }, { "verbose", 1, NULL, 'v' }, {
					"noresample", 1, NULL, 'n' }, { "pevent", 1, NULL, 'e' }, {
			NULL, 0, NULL, 0 }, };
	snd_pcm_t *handle;
	int err, morehelp;
	...
	if ((err = set_hwparams(handle, hwparams, transfer_methods[method].access))
			< 0) {
